...
class Validator extends OsmFunctions
{
	...
	protected function time($st)
	{
		...
		if (is_array($st)) {
			...
			if ($st != '24/7' && preg_match('/[^\d:-a-z -,]/i', $st))
				$st = '';
	...
	{
		$st = preg_split('/[,;\/]/', $st);
		$st = $st[0]; // возможно несколько телефонов, берем первый
		...
		$len = strlen($st);
		if ($len <= 12 || $len > 14)
			$st = ''; // что-то пошло не так: получился короткий номер +7-000-12345
