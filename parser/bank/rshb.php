...
class rshb extends Validator
{
	protected $domain = 'https://www.rshb.ru/api/v1/branchesAndRemoteWorkplaces?regionCode=';
	...
	protected $filter = [
		'[amenity=bank][name~"Россельхозбанк",i]'
	];
	...
	protected function parse($st)
	{
		...
		foreach ($st['branches'] as $obj) {
			...
			// Идентификатор
			$obj['ref'] =  $obj['registrationNumber'];
			...
			// Время работы
			if (!empty($obj['workSchedule']['workTimeSaturday'])) {
				$workTime = 'Mo-Fr '.$obj['workSchedule']['workTime'].'; Sa '.$obj['workSchedule']['workTimeSaturday'];
			} else {
				$workTime = 'Mo-Fr '.$obj['workSchedule']['workTime'];
			}

			$obj['opening_hours'] = $this->time($workTime);

			// Доступность для инвалидных колясок
			if ($obj['equippedWithRamp'] == 'true') {
				$obj['wheelchair'] = 'yes';
			} else {
				$obj['wheelchair'] = 'no';
			}
